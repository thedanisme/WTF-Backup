...
function bdlc:startSession(itemLink,num)
	...
	if (GetItemInfo(itemLink)) then
		...
		local isRelic = select(7, GetItemInfo(itemLink)) == EJ_LOOT_SLOT_FILTER_ARTIFACT_RELIC and true or false
		...
		if (not bdlc.loot_sessions[itemUID] and ((equipSlot and string.len(equipSlot) > 0) or found_tier or isRelic)) then
...
function bdlc:startMockSession()
	...
	bdlc:buildLC()
	bdlc.item_drops = {
		[GetInventoryItemLink("player", 6)] = 1,
		[GetInventoryItemLink("player", 3)] = 3,
		[GetInventoryItemLink("player", 7)] = 1,
		[GetInventoryItemLink("player", 2)] = 2
	}
...
function bdlc:createVoteWindow(itemUID,num)
	...
	if (tonumber(gem) > 0 or tonumber(bonus1) == 3) then
...
function bdlc:createRollWindow(itemUID,num)
	...
	if (quality < 6 and (tonumber(gem) > 0 or tonumber(bonus1) == 3)) then
	...
	--else
		--bdlc:removeUserRoll(itemUID)
		--SendAddonMessage(bdlc.message_prefix, "removeUserRoll><"..itemUID, "WHISPER", playerName);
		--SendAddonMessage(bdlc.message_prefix, "removeUserConsidering><"..bdlc.local_player.."><"..itemUID, "WHISPER", UnitName("player"));
		--bdlc:removeUserRoll(itemUID)
	--end
...
-- UpdateUserItem
----------------------------------------
function bdlc:updateUserItem(itemUID, frame)
	local itemLink = bdlc.itemUID_Map[itemUID]
...
bdlc:SetScript("OnEvent", function(self, event, arg1, arg2, arg3)
	...
	if (event == "PLAYER_ENTERING_WORLD" or event == "LOADING_SCREEN_DISABLED") then
		...
		SendAddonMessage(bdlc.message_prefix, "fetchSessions", bdlc.sendTo, UnitName("player"));
		if (IsInRaid()) then
			bdlc.sendTo = "RAID"
		else
			bdlc.sendTo = "WHISPER"
