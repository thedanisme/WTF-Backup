        ...
        if count > 1 then -- I must have at least 2 lines in tooltip
          ...
          if type(pattern) == "number" then
            if count >= pattern then
              ...
              if text and text ~= "none" then NOP.T_RECIPES_FIND[itemID] = {c,text} end
            end
          elseif type(pattern) == "string" then
            ...
            if heading then -- look in 1st line
              ...
              if compare ~= heading then
                NOP.T_RECIPES_FIND[itemID] = {c,compare}
              end
            end
          end
        else
...
function NOP:SpellLoad() -- load spell patterns
  ...
  for spellid,data in pairs(NOP.T_SPELL) do
    ...
    if spellName and count > 1 then -- it has spell and tooltip has at least 2 lines
