...
[list][*]Create single button to click on to combine/open/use item in bags. 
[*]*NEW*Create button bar to click on for quest items.
[*]Button can have assigned hot-key to be able use keyboard short-cut, via Game Menu -> Key Bindings. 
[*]When click to use item add-on will continue to scan bags and change to next usable item in your inventory.
[*]Add-on take care if you have enough items to combine if not then item is not assigned to button.
[*]If there is no usable item known by add-on it will hide button. Except when is configured to show fake button (good for repositioning)
[*]Is possible skip item (remove from button) by right-click, when no other items are found then all skipped items are placed back on button. Skip can be configured to last whole session until relog. It is configurable via slash "/nop skip" or from Game Menu -> Interface -> AddOns.
...
[*]Few items can be used only in proper zone. Two items are designed only for garrison mine. Some items can be used only in "Salvage yard".
[*]*NEW* Quest items. For each quest item in bags is added separate button. Direction and number buttons per row can be configured in Game Menu -> Interface -> AddOns. This part is taken from Quest Item Bar made by Nickenyfiken and ZidayaXis. I loved that add-on and original authors have no plans to maintain it. I did adopted core functionality from that add-on.[/list]

]]
--[[Supported items:

[list][*]All items used with standard "Open" spell
[*]Archaeology artifacts
[*]Leather-working scraps (Ruined, Knothide, Borean, Savage, Raw Beast)
[*]Salvage bags, armor and weapons boost from follower’s missions
[*]Pet Supplies
[*]Items where is possible combine lesser one items into something else
[*]WoD fishing catches
[*]Crafting plans,recipes,designs etc. in fact all learnable profession items
[*]Looted items generating garrison resources on use
[*]Scribe's Research Notes and Abrogator Stone Cluster
[*]Miner's Coffee (number of stacks can be configured in options) and Preserved Mining Pick in Garrison mine
[*]Pets and toys are added.
[*]Rogues now have placed locked lock-boxes on button, according to level of lock-picking skill. 1st click lock-pick and second click open. Can be configured in options, is enabled by default[/list]
...
Actually button can be skinned (see screen shots), by default skin is disabled. Skinning can be toggled via Game Menu -> Interface -> AddOns or via slash command "/nop skin". If you move button to place where you can't see him, or move, then it can be reset to middle of screen via slash command "/nop reset". Is possible lock in place button via Game Menu -> Interface -> AddOns or slash "/nop lock".

Original idea behind this come from Driizt@BB_EU
