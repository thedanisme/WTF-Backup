...
function TA.Initialise()
	...
	hooksecurefunc("PlayerTalentFrame_ShowPVPTalentTab",TA.SetPVPTalents)
	...
	for i=1,MAX_PVP_TALENT_TIERS do
		local blizz_talent=PlayerTalentFramePVPTalents.Talents["Tier"..i]["Talent1"]
		local icon = CHAIN(CreateFrame("Button", "ZygorTAPVPTalent"..i, blizz_talent))
			:SetSize(20,20)
			:SetPoint("TOPRIGHT")
			:SetNormalTexture(ZGV.DIR.."\\Skins\\zygor-round")
			:SetFrameStrata("HIGH")
			:SetFrameLevel(76)
			:SetScript("OnEnter", function(self) TA.ShowTooltip(self) end)
			:Hide()
		.__END
		icon:GetNormalTexture():SetTexCoord(0,0,0,1/2 , 1,0,1,1/2)
		TA.PVPTalentIcons[i]=icon
	end
...
function TA.SetPVPTalents() 
	...
	local suggested_spec
	if TA.PVPBuilds[TA.PlayerClass] then
		for _,specdata in pairs(TA.PVPBuilds[TA.PlayerClass]) do
			if specdata[1]==spec then
				suggested_spec = specdata
			end
		end
	...
	local talents = suggested_spec[2]

	for row,talent in pairs(talents) do
		local blizz_talent = PlayerTalentFramePVPTalents.Talents["Tier"..row]["Talent"..talent]
		local icon = TA.PVPTalentIcons[row]
		icon:SetParent(blizz_talent)
		icon:ClearAllPoints()
		icon:SetPoint("TOPRIGHT")
		icon:Show()
