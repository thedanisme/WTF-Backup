...
function IM:isItemSoulbound(bagID, bagSlotID)
	...
	TooltipFrame:SetBagItem(bagID,bagSlotID)
	if IM:TooltipsContain(nil, ITEM_SOULBOUND) then
		return true
	else
		return false
	end
...
tinsert(ZGV.startups,{"InventoryManager setup",function(self)
	...
	IM:Debug(type(ZGV.db.profile.visible))
	if db.lastParentName == "Zygor Guide" and ZGV.db.profile.visible ~= false and ZGV.db.profile.im_enable and ZGV.db.profile.enable_vendor_tools then
		ZGV.IM.InventoryManagerFrame:Show()
	end
