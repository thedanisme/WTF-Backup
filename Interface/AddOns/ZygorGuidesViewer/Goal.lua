...
function Goal:IsComplete()
	...
	if GOAL and GOAL.iscomplete then
		...
		return iscomplete,ispossible or self.future,progress,"fallback '".. self.action .. "'" .. (comment and ": ".. comment  or  ""),isbad
	end

	return false,false or self.future, nil , "complete check failed"
end
...
GOALTYPES['achieve'] = {
	...
	iscomplete = function(self)
		if self.achievesub then
			...
			return not not completed, (quantity or 0), quantity/required
		else
			-- full achievement
			...
			return not not completed, true, numcrit>0 and completenum/numcrit or 0, completed and "achieve" or "FALLTHROUGH"
		end
	end,
	gettext = function(self)
		if self.achievesub then
...
GOALTYPES['poiachievement'] = {
	parse = function(self,params,step)
		...
		if self.poisubachieve then
			self.poisubachievename = (GetAchievementCriteriaInfo(self.poiachieve,self.poisubachieve))
