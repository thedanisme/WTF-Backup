...
function ZGV:QuestTracking_CacheQuestLog(from)
	...
	local nc=0
	for i = 1,50 do
		local sQuestLogTitleText, iQuestLevel, iSuggestedGroup, isHeader, isCollapsed, isComplete, frequency, questID, startEvent, displayQuestID, isOnMap, hasLocalPOI, isTask, isStory = GetQuestLogTitle(i)
...
function ZGV:MarkUselessQuests()
	...
	for qi,quest in ipairs(self.quests) do
		quest.useless = not guidequests[quest.id] and not quest.daily and not (quest.tagnum==102) and not (quest.tagnum==83) and not (quest.tagnum==89)
