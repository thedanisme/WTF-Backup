...
step
#include "H_Garrison_Finalize_Small", building="Alchemy"
Finalize the Level 1 Alchemy Lab |havebuilding Alchemy,1,active
step
#include "H_Garrison_Small_Building", action="talk Albert de Hyde##79813", building="Alchemy"
...
step
#include "H_Garrison_Small_Building", action="talk Keyana Tone##79814", building="Alchemy"
...
step
#include "H_Garrison_Small_Building", action="click Alchemy Work Order##235892", building="Alchemy"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Enchanting"
Finalize the Level 1 Enchanter's Study |havebuilding Enchanting,1,active
step
#include "H_Garrison_Small_Building", action="talk Yukla Greenshadow##79821", building="Enchanting"
...
step
#include "H_Garrison_Small_Building", action="talk Garra##79820", building="Enchanting"
Tell her _"I would like to place a work order."_
Click _Start Work Order_
Start the Enchanting Work Order |q Your First Enchanting Work Order##37570/1
step
#include "H_Garrison_Small_Building", action="click Enchanting Work Order##237138", building="Enchanting"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Engineering"
Finalize the Level 1 Engineering Works |havebuilding Engineering,1,active
step
#include "H_Garrison_Small_Building", action="talk Pozzlow##79826", building="Engineering"
...
step
#include "H_Garrison_Small_Building", action="talk Garbra Fizzwonk##86696", building="Engineering"
Tell her _"I would like to place a work order"_
Click _Start Work Order_
Start the Engineering Work Order |q Your First Engineering Work Order##37571/1
step
#include "H_Garrison_Small_Building", action="click Engineering Work Order##237146", building="Engineering"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Blacksmith"
Finalize the Level 1 Forge |havebuilding Blacksmith,1,active
step
#include "H_Garrison_Small_Building", action="talk Orgek Ironhand##79867", building="Blacksmith"
...
|tip These can be found around the cliffs in this area. They can also be mined in your Frostwall Mines, if you have one.
step
#include "H_Garrison_Small_Building", action="talk Kinja##79817", building="Blacksmith"
Tell her _"I would like to place a work order"_
Click _Start Work Order_
Start the Blacksmithing Work Order |q Your First Blacksmithing Work Order##37569/1
step
#include "H_Garrison_Small_Building", action="click Blacksmithing Work Order##236641", building="Blacksmith"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Jewelcrafting"
Finalize the Level 1 Gem Boutique |havebuilding Jewelcrafting,1,active
step
#include "H_Garrison_Small_Building", action="talk Dorogarr##79832", building="Jewelcrafting"
...
step
#include "H_Garrison_Small_Building", action="talk Elrondir Surrion##79830", building="Jewelcrafting"
Tell her _"I would like to place a Work Order"_
Click _Start Work Order_
Start the Jewelcrafting Work Order |q Your First Jewelcrafting Work Order##37573/1
step
#include "H_Garrison_Small_Building", action="click Jewelcrafting Work Order##236652", building="Jewelcrafting"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="SalvageYard"
Finalize the Level 1 Salvage Yard |havebuilding SalvageYard,1,active
step
#include "H_Garrison_Small_Building", action="talk Lumba the Crusher##79857", building="SalvageYard"
...
step
#include "H_Garrison_Small_Building", action="In your Salvage Yard:", building="SalvageYard"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Inscription"
Finalize the Level 1 Scribe's Quarters |havebuilding Inscription,1,active
step
#include "H_Garrison_Small_Building", action="talk Urgra##79829", building="Inscription"
...
step
#include "H_Garrison_Small_Building",action="talk Y'rogg##79831", building="Inscription"
...
step
#include "H_Garrison_Small_Building",action="click Inscription Work Order##236649", building="Inscription"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Storehouse"
Finalize the Level 1 Storehouse |havebuilding Storehouse,1,active
step
#include "H_Garrison_Small_Building", action="talk Yorn Longhoof##79862", building="Storehouse"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Tailoring"
Finalize the Level 1 Tailoring Emporium |havebuilding Tailoring,1,active
step
#include "H_Garrison_Small_Building", action="talk Warra the Weaver##79864", building="Tailoring"
...
step
#include "H_Garrison_Small_Building", action="talk Turga##79863", building="Tailoring"
Tell her _"I would like to place a work order"_
Click _Start Work Order_
Start the Tailoring Work Order |q Your First Tailoring Work Order##37575/1
step
#include "H_Garrison_Small_Building", action="click Tailoring Work Order##237665", building="Tailoring"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Small", building="Leatherworking"
Finalize the Level 1 Tannery |havebuilding Leatherworking,1,active
step
#include "H_Garrison_Small_Building", action="talk Murne Greenhoof##79834", building="Leatherworking"
...
step
#include "H_Garrison_Small_Building", action="talk Yanney##79833", building="Leatherworking"
Tell her _"I would like to place a Work Order"_
Click _Start Work Order_
Start the Leatherworking Work Order |q Your First Leatherworking Work Order##37574/1
step
#include "H_Garrison_Small_Building", action="click Leatherworking Work Order##236948", building="Leatherworking"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Medium", building="Barn"
Finalize the Level 1 Barn |havebuilding Barn,1,active
step
#include "H_Garrison_Medium_Building", action="talk Farmer Lok'lub##85048", building="Barn"
...
step
#include "H_Garrison_Medium_Building", action="click Iron Trap##234186", building="Barn"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Medium", building="Inn"
...
step
#include "H_Garrison_Medium_Building", action="talk Akanja##87305", building="Inn"
...
step
#include "H_Garrison_Medium_Building", action="talk Millhouse Manastorm##88009", building="Inn"
|tip He's upstairs in the Frostwall Tavern.
accept For The Children!##37179
step
label "Start_Inn_Quests"
#include "H_Garrison_Medium_Building", action="Accept any quests in your Frostwall Tavern", building="Inn"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Medium", building="SparringArena"
Finalize the Level 1 Gladiator's Sanctum |havebuilding SparringArena,1,active
step
#include "H_Garrison_Medium_Building", action="talk Raza'kul##86629", building="SparringArena"
...
step
#include "H_Garrison_Medium_Building", action="clicknpc Altar of Bones##86639", building="SparringArena"
...
step
#include "H_Garrison_Medium_Building", action="click Nemesis", building="SparringArena"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Medium", building="LumberMill"
Finalize the Level 1 Lumber Mill |havebuilding LumberMill,1,active
step
#include "H_Garrison_Medium_Building", action="talk Lumber Lord Oktron##84247", building="LumberMill"
...
step
#include "H_Garrison_Medium_Building", action="talk Alexi Barov##85414", building="LumberMill"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Medium", building="TradingPost"
Finalize the Level 1 Trading Post |havebuilding TradingPost,1,active
step
#include "H_Garrison_Medium_Building", action="talk Fayla Fairfeather##87120", building="TradingPost"
...
step
#include "H_Garrison_Medium_Building", action="talk Ancient Trading Mechanism##87206", building="TradingPost"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Large", building="Barracks"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Large", building="Workshop"
Finalize the Level 1 Goblin Workshop |havebuilding Workshop,1,active
step
#include "H_Garrison_Large_Building", action="talk Vee##85369", building="Workshop"
|tip She's in your Goblin Workshop garrison building.
accept Unconventional Inventions##37044
step
#include "H_Garrison_Large_Building", action="click Sticky Grenades##234146", building="Workshop"
...
#include "H_Garrison_Large_Building", action="click Invention", building="Workshop"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Large", building="MageTower"
Finalize the Level 1 Spirit Lodge |havebuilding MageTower,1,active
step
#include "H_Garrison_Large_Building", action="talk Gez'la##79836", building="MageTower"
|tip She's inside the tent.
accept Portable Portals##38351
step
#include "H_Garrison_Large_Building", action="talk Varsha##91577", building="MageTower"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Large", building="Stables"
...
step
#include "H_Garrison_Large_Building", action="talk Sage Paluna##87242", building="Stables"
...
step
#include "H_Garrison_Large_Building", action="click Black Claw of Sethe##237169", building="Stables"
...
step
#include "H_Garrison_Large_Building", action="talk Tormak the Scarred##86979", building="Stables"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Finalize_Large", building="Armory"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_Fishing_Finalize"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_FrostwallMines_Finalize"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
#include "H_Garrison_HerbGarden_Finalize"
...
|tip This takes 1 hour from the time you initiated the upgrade.
step
click Finalize Garrison Plot |goto Frostwall/0 33.5,41.6
...
step
click Naval Equipment Blueprint: Blast Furnace
...
modelnpc Sethekk Dreadclaw##94467
modelnpc Sethekk Invoker##94468
modelnpc Sethekk Oracle##94469
modelnpc Eye of the Discordant##90655
step
talk Reshad##89400
...
modelnpc Bladespire Goresnout##75241
modelnpc Bladespire Brute##73371
modelnpc Bladespire Stonecaster##73472
step
...
step
click Barrel of Frostwolf Oil##334+
...
modelnpc Thunderlord Crag-Leaper##84812
modelnpc Thunderlord Giantslayer##78688
modelnpc Thunderlord Gronnstalker##84815
step
talk Ga'nar##74000
...
accept Call of the Archmage##35988 |goto Frostwall/0 47.1,47.2
step
...
collect Pure Solium Band##114780 |q Spires of the Betrayer##36157/1 |goto Skyreach/2 52.9,25.2
step
talk Archmage Khadgar##83823
|tip Right click the green eye icon on your mini map and leave the dungeon.
...
Speak With Khadgar |q Khadgar's Task##35989/1 |goto Talador/0 85.2,31.7
step
talk Archmage Khadgar##83823
...
stickystart "secretapexis"
step
Press _I_ and queue for _The Everbloom HEROIC_
kill Yalnu##83846
|tip Yalnu is the final boss of the dungeon.
collect Core of Life##114138 |q Core of Life##35992/1 |goto The Everbloom/2 44.6,70.0
step
Press _I_ and queue for _Grimrail Depot HEROIC_
click Engine Access##233775
|tip This is found at the very end of the dungeon.
...
collect Core of Flame##113682 |q Core of Flame##35990/1 |goto Bloodmaul Slag Mines/1 67.9,22.8
step
talk Archmage Khadgar##83823
...
click Corrupted Blood of Teron'gor##237542
|tip This is found on the ground after defeating Teron'gor.
collect Corrupted Blood of Teron'gor##114240 |q Tackling Teron'gor##35993/1 |goto Auchindoun 48.4,30.3
step
talk Archmage Khadgar##83823
turnin Tackling Teron'gor##35993 |goto Talador/0 85.16,31.69
step "secretapexis"
earn 4985 Apexis Crystal##823 |q Draenor's Secret Power##36158/1
|tip These can be gained from Garrison dailies, Garrison missions, Tanaan dailies, and level 100 enemies.
step
talk Archmage Khadgar##83823
...
Wait For Khadgar to Complete the Spell |q Eyes of the Archmage##35994/1 |goto Talador/0 85.16,31.69
step
talk Archmage Khadgar##83823
...
step
Find the First Clue |q The Dragon's Tale##36206/1 |goto Nagrand D/0 78.5,35.5
Find the Second Clue |q The Dragon's Tale##36206/2 |goto Nagrand D/0 81.1,35.5
_Follow the path_ across the stone bridge |goto Nagrand D/0 82.1,37.1 < 15 |only if walking
Find the Third Clue |q The Dragon's Tale##36206/3 |goto Nagrand D/0 83.3,36.5
only if not completedq(36206)
step
_Go up_ the narrow path |goto Nagrand D/0 85.0,34.7 < 15 |only if walking
Find the Fourth Clue |q The Dragon's Tale##36206/4 |goto Nagrand D/0 86.7,38.2
Find the Fifth Clue |q The Dragon's Tale##36206/5 |goto Nagrand D/0 89.9,35.6
...
step
kill Spirit of Kairozdormu##84744
|tip Run behind one of the rocks when he casts Ripples in Time.
Slay the Spirit of Kairozdormu |q Tarnished Bronze##35995/1 |goto Nagrand D/0 92.4,31.7
step
talk Archmage Khadgar##84702
...
Wait For Khadgar to Complete the Spell |q Power Unleashed##36004/2 |goto Talador/0 85.2,31.7
step
talk Archmage Khadgar##83823
...
stickystart "highmaul"
step
Press _I_ and queue for _Arcane Sanctum_
|tip This can also be done with a raid group on Normal, Heroic or Mythic difficulty.
kill Ko'ragh##79015
|tip Ko'ragh is the final boss of the Arcane Sanctum section of the Highmaul raid.
collect Felbreaker's Tome##115288 |q Empire's Fall##35997/1 |goto Highmaul/4 37.80,65.4
step
Press _I_ and queue for _Imperator's Rise_
|tip This can also be done with a raid group on Normal, Heroic or Mythic difficulty.
kill Imperator Mar'gok##87818
|tip Imperator Mar'gok is the only boss of the Imperator's Rise section of the Highmaul raid.
collect Sigil of the Sorcerer King##115289 |q Empire's Fall##35997/2 |goto Highmaul/6 45.3,75.0
step "highmaul"
collect 125 Abrogator Stone##115280 |q Legacy of the Sorcerer Kings##35998/1
|tip These can be looted from the Highmaul raid, rare Garrison Missions, and sometimes in Work Orders.
step
talk Archmage Khadgar##83823
...
Wait For Khadgar to Complete the Spell |q The Scrying Game##36005/1 |goto Talador/0 85.1,31.6
step
...
step
map Talador/0
path follow strict; loop off; ants straight; dist 10
path	83.9,30.4	83.9,29.4	84.8,29.1
path	85.6,29.3	86.2,29.6	86.4,30.7
path	86.8,32.2
Track Garona! |goto Talador/0 86.8,32.2 < 10 |noway |c
|tip Follow the green splotches on the ground. You can abandon and restart the quest if you need to. Don't attack Garona until you're at the cave!
only if havequest(36006)
step
Track Garona Down |q Hunter: Hunted##36006/2 |goto Talador/0 87.8,32.7
step
map Talador/0
path follow strict; loop off; ants straight; dist 10
path	87.1,32.3	86.5,31.2	85.8,29.2
path	84.3,28.1
Pursue Garona! |goto Talador/0 84.3,28.1 < 10 |noway |c
|tip Follow the path as you pursue her.
only if havequest(36006)
step
Corner Garona |q Hunter: Hunted##36006/3 |goto Talador/0 84.1,28.9
step
kill Garona Halforcen##85134
Defeat Garona |q Hunter: Hunted##36006/4 |goto Talador/0 84.0,29.9
step
talk Garona Halforcen##85134
Select "_<Use the Incarceration Rune>_"
...
Wait For Khadgar to Complete the Spell |q Touch of the Kirin-Tor##36007/1 |goto Talador/0 85.1,31.7
step
talk Archmage Khadgar##84702
...
stickystart "might"
step
Press _I_ and queue for _Slagworks_
|tip This can also be done with a raid group on Normal, Heroic or Mythic difficulty.
kill Foreman Feldspar##76809
|tip Foreman Feldspar is the final boss of the Slagworks section of the Blackrock Foundry raid.
collect Heart of the Fury##115479 |q Heart of the Fury##36009/1 |goto Blackrock Foundry/2 47.0,54.0
step
Press _I_ and queue for _The Black Forge_
|tip This can also be done with a raid group on Normal, Heroic or Mythic difficulty.
kill Flamebender Ka'graz##76814
|tip Flamebender Ka'graz is the final boss of the Black Forge section of the Blackrock Foundry raid.
collect Flamebender's Tome##115493 |q Flamebender's Tome##36010/1 |goto Blackrock Foundry/1 19.3,76.8
step
Press _I_ and queue for _Iron Assembly_
|tip This can also be done with a raid group on Normal, Heroic or Mythic difficulty.
click Stolen Sha'tari Vault##234745
|tip It's a large metal safe that is lootable after killing the Iron Maidens, the final boss of the Iron Assembly section of the Blackrock Foundry raid.
...
step
Press _I_ and queue for _Blackhand's Crucible_
|tip This can also be done with a raid group on Normal, Heroic or Mythic difficulty.
kill Blackhand##77325
|tip Blackhand is the only boss in the Blackhand's Crucible section of the Blackrock Foundry raid.
...
step
label "might"
collect 900 Elemental Rune##115510 |q 36013
|tip These can be gained through killing raid bosses in Blackrock Foundry on any difficulty, completing rare Garrison missions, and completing work orders.
step
...
accept The Unbroken Circle##36016 |goto Talador/0 85.17,31.70
step
talk Archmage Khadgar##83823
Tell him "_I am ready, Archmage._"
Wait For Khadgar to Complete the Spell |q The Unbroken Circle##36016/1 |goto Talador/0 85.1,31.7
step
talk Archmage Khadgar##83823
turnin The Unbroken Circle##36016 |goto 85.18,31.72
accept Prisoner of the Mind##36017 |goto 85.18,31.72
step
talk Archmage Khadgar##83823 |goto Talador/0 85.2,31.7 < 5
Tell him "_I am ready. Let's free Garona's mind!_"
_Follow Khadgar_ to this spot |goto Talador/0 84.6,30.3 < 5
|tip Run here after talking to Khadgar.
Wait For Khadgar to Complete the Spell |q Prisoner of the Mind##36017/1 |goto Talador/0 84.6,30.2
step
talk Archmage Khadgar##83823
turnin Prisoner of the Mind##36017 |goto 85.18,31.72
accept Orb of Dominion##37834 |goto 85.17,31.71
step
Locate the Den of Secrets |q Orb of Dominion##37834/1 |goto Shadowmoon Valley D/0 54.8,15.1
step
Click the _Quest Box_:
accept The Shadow War##37835 |goto 54.86,15.18
stickystart "shadowwar"
step
_Enter_ the cave |goto Shadowmoon Valley D/0 55.0,15.8 < 10 |walk
kill Dentarg##90549, Zaargleen##90551, Elodor Crystalsmith##81002
click Orb of Dominion##3365
collect Orb of Dominion##122155 |q Orb of Dominion##37834/2 |goto Shadowmoon Valley D/22 49.7,76.5
step
label "shadowwar"
kill Dark Enforcer##88663+, Shadow Council Ritualist##88664+
Slay #15# Shadow Council Members |q The Shadow War##37835/1 |goto Shadowmoon Valley D/22 36.4,58.9
step
talk Archmage Khadgar##83823
turnin Orb of Dominion##37834 |goto Talador/0 85.17,31.70
accept Breaking Badness##37836 |goto 85.16,31.70
turnin The Shadow War##37835 |goto 85.17,31.70
step
talk Archmage Khadgar##83823 |goto Talador/0 85.2,31.7 < 5
Tell him "_Let's try this again. For Garona!_"
_Follow Khadgar_ to this spot |goto Talador/0 84.6,30.3 < 5
|tip Run here after talking to Khadgar.
kill Garona's Demon Within##90674
Free Garona's Mind |q Breaking Badness##37836/1 |goto Talador/0 84.6,30.2
step
talk Archmage Khadgar##83823
turnin Breaking Badness##37836 |goto 85.18,31.72
accept To Gul'dan!##37964 |goto 85.17,31.71
step
talk Archmage Khadgar##83823
turnin To Gul'dan!##37964 |goto 67.46,6.55
accept An Inside Job##37837 |goto 67.46,6.55
step
Use Garona's _Assassinate_ ability
|tip Use this ability on enemies before they sound the alarm. Try to use it when there aren't other enemies nearby. Often, you'll be able to Assassinate successfully when other nearby enemies have their backs turned or are walking away.
_Approach_ the gate |goto Talador/0 68.0,6.3 < 5
|tip Wait here for the first guard to move up, Assassinate him, and them continue forward.
Infiltrate the Compound |q An Inside Job##37837/1 |goto Talador/0 68.4,5.5
step
_Enter_ the building |goto Talador/0 69.8,3.0 < 10 |walk
|tip Continue working your way forward, Assassinating enemies when they are alone.
click Iron Horde Missives##239978
|tip It looks like a small brown box filled with rolled up parchment sitting on the table.
Locate the Iron Horde Intel |q An Inside Job##37837/2 |goto Talador/0 70.8,2.9
step
_Move up_ the hill |goto Talador/0 69.4,4.5 < 15 |walk
click Convenient Hiding Spot##240010
|tip It's a huge pile of crates and barrels.
Spy On Gul'dan |q An Inside Job##37837/3 |goto Talador/0 69.0,5.4
step
talk Archmage Khadgar##83823
turnin An Inside Job##37837 |goto 67.46,6.55
accept The Final Assault##37841 |goto 67.46,6.55
step
talk Garona Halforcen##90237
accept My Garona##37838 |instant |goto Talador/0 67.5,6.6
only if not hasfollower(466)
step
talk Archmage Khadgar##92213
turnin The Final Assault##37841 |goto Frostfire Ridge/0 40.85,73.33
|tip You must complete the quests to build your Frostwall Shipyard before you can turn this in.
|tip Use the Frostwall Shipyard building guide to accomplish this.
step
talk Archmage Khadgar##92213
accept Command of the Seas##39057 |goto Frostfire Ridge/0 40.85,73.33
accept We Don't Need No Library Card##39000 |goto Frostfire Ridge/0 40.85,73.33
step
talk Rolosh Wavechaser##94789
accept Upgrading The Fleet##39245 |goto Frostfire Ridge/0 42.1,71.0
stickystart "chaostome"
step
clicknpc Fleet Command Table##94398
|tip Use the Command Table to send your ships on missions.
Complete 15 Naval Missions |q Upgrading The Fleet##39245/3 |goto Frostfire Ridge/0 42.0,71.0
step
Earn 1000 Garrison Resources |q Upgrading The Fleet##39245/1
Collect 1000 Gold |q Upgrading The Fleet##39245/2
|tip These are needed to upgrade your Shipyard to Level 2.
step
talk Rolosh Wavechaser##94789
turnin Upgrading The Fleet##39245 |goto Frostfire Ridge/0 42.1,71.0
step
talk Archmage Khadgar##92213
turnin Command of the Seas##39057 |goto Frostfire Ridge/0 40.85,73.33
accept Master vs. Commander##39697 |goto Frostfire Ridge/0 40.85,73.33
step
clicknpc Fleet Command Table##94398
Acquire a Draenic Sea Chart |q Master vs. Commander##39697/1 |goto Frostfire Ridge/0 42.0,71.0
|tip This is gained by completing the Boarding the Master's Call legendary Naval Mission.
|tip This mission will appear after completing the other 4 legendary Naval Missions: Stalking the Master, Giving Chase, Ambush on the High Seas, and In Hot Pursuit.
step
label "chaostome"
collect 33 Tome of Chaos##127115 |q We Don't Need No Library Card##39000/1
|tip These can be gained through killing raid bosses in Hellfire Citadel in LFR or Normal, Heroic, and Mythic difficulties.
step
talk Archmage Khadgar##92213
turnin We Don't Need No Library Card##39000 |goto Frostfire Ridge/0 40.85,73.33
accept Cooking the Books##39003 |goto Frostfire Ridge/0 40.85,73.33
turnin Master vs. Commander##39697 |goto Frostfire Ridge/0 40.85,73.33
step
Travel to Zangarra |q Cooking the Books##39003/1 |goto Talador/0 84.8,31.4
step
talk Cordana Felsong##83929
Tell her _"Khadgar wants you to destroy these books."_
Give the Books to Cordana |q Cooking the Books##39003/2 |goto Talador/0 85.2,31.7
step
talk Cordana Felsong##83929
Tell her _"Oh, HELL no!"_
kill Tome of Chaos##94156+, Cordana Felsong##94054
|tip Kill the Tomes of Chaos to break Cordana's invulnerability.
Confront Cordana |q Cooking the Books##39003/3 |goto Talador/0 85.2,31.7
step
talk Archmage Khadgar##92213
turnin Cooking the Books##39003 |goto Frostfire Ridge/0 40.85,73.33
accept Draenor's Blessing##37840 |goto 40.84,73.31
step
talk talk 82070
Tell him _"I am ready, Farseer."_
Receive the Blessing of the Elements |q Draenor's Blessing##37840/1 |goto Nagrand D/0 71.9,19.2
step
talk Archmage Khadgar##92213
turnin Draenor's Blessing##37840 |goto Nagrand D/0 72.0,19.3
accept Darkness Incarnate##39004 |goto 72.02,19.26
step
Press _I_ and queue for _The Black Gate_
|tip This can also be done with a raid group on Normal, Heroic or Mythic difficulty.
kill Archimonde##91331
|tip Archimonde is the only boss in the Black Gate section of the Hellfire Citadel raid.
Thwart Gul'dun's Plans |q Darkness Incarnate##39004/1 |goto Hellfire Citadel/10 58.4,53.3
step
talk Archmage Khadgar##92213
turnin Darkness Incarnate##39004 |goto Frostfire Ridge/0 40.85,73.33
step
Congratulations, you have completed the Legendary Ring questline!
...
step
#include "H_Garrison_Medium_Building", action="talk Kil'rip##86698", building="TradingPost"
...
modelnpc Vian the Voltile##67579
step
...
step
click Dark Iron Mole Machine##10497 |goto Gorgrond/0 60.7,64.8 < 5
