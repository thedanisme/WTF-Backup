...
function Gold:GetSellPrice(itemid,itemCount,usingFaked)
	...
	if itemtrend then  -- either local or global
		...
		if (undercutprice or 0) > 0 then
			if undercutprice > highPrice*Gold.OVERPRICE then -- Prices gouged, sell now!
				priceOverride = highPrice*Gold.OVERPRICE
			end
		else
			--if emptyMarket or errortext=="no data for item" then -- Market is empty, sell now!
				priceOverride = highPrice*Gold.OVERPRICE
			--end
		end
	end
