...
local MAJOR = "LibStatLogic-1.2-Dugi";
...
function StatLogic:SetTip(item)
	local name, link, _, _, reqLv, _, _, _, itemType = GetItemInfo_dugi(item)
	if not link then
...
if not tip then
    ...
    --tip:SetSize(228,228)
	--tip:SetPoint("TOPLEFT",UIParent,"TOPLEFT", 145, 2)
	for i = 1, 40 do
...
local CLASSID_DEMONHUNTER = 12;
local CLASSID_MAX = CLASSID_DEMONHUNTER; --the highest valid classID
...
--	[CR_WEAPON_SKILL] = "WEAPON_RATING",
	...
	[CR_MULTISTRIKE] = "MULTISTRIKE_RATING",
	[CR_LIFESTEAL] = "LEECH_RATING",
	[CR_AVOIDANCE] = "AVOIDANCE_RATING",
	...
	["MULTISTRIKE_RATING"] = CR_MULTISTRIKE,
	["LEECH_RATING"] = CR_LIFESTEAL,
	["AVOIDANCE_RATING"] = CR_AVOIDANCE,	
}
...
function StatLogic.GetArmorSpecActive()
	return ArmorSpecActive
end
...
if playerArmorType then
	...
	local function UpdateArmorSpecActive()
		...
		for _, slot in pairs(WatchInventoryID) do
			...
			if (select(7, GetItemInfo_dugi(item))) ~= playerArmorType then return end
		end
		-- all pass
...
elseif playerClass == "DEATHKNIGHT" then
	...
	--Hotfix (2013-03-05): "Might of the Frozen Wastes now increases melee attack damage by 20% (was 15%)."
	["MOD_DMG"] = {
		{
			["rank"] = {
				0.2,
...
elseif playerClass == "MONK" then
	...
	-- Monk: Swift Reflexes - Passive: 124334
	...
	-- Monk: Stance of the Fierce Tiger - Stance: "Interface\\Icons\\monk_stance_whitetiger"
	-- Increases your movement speed by 10%, increases damage done by 20% and increases the amount of Chi generated by your Jab and Expel Harm abilities by 1.
	["MOD_DMG"] = {
		["rank"] = {
          0.2,
        },
        ["stance"] = "Interface\\Icons\\monk_stance_whitetiger",
	},
	
	-- Stance of the Sturdy Ox - Brewmaster Stance: "Interface\\Icons\\monk_stance_drunkenox"
	...
	-- Stance of the Wise Serpent - Mistweaver Stance: "Interface\\Icons\\monk_stance_wiseserpent"
...
local function IsMetaGemActive(item)
	...
	local name, link, _, _, _, _, itemSubType = GetItemInfo_dugi(item)
...
-----------------------------------]]
local buffGroup = {}
function StatLogic:GetStatMod(stat, school, talentGroup, modTable, armorSpecOverride)
    ...
    if type(StatModTable[playerClass][stat]) == "table" and modTable ~= "ALL" then
        ...
        for _, case in ipairs(StatModTable[playerClass][stat]) do
			...
			if case.armorspec and case.armorspec==armorSpecOverride then ok=true end
...
local H = {
  ...
  [108] = 6056.4549480855 ,
  [109] = 7920.8626856203 ,
  [110] = 10359.2062059811,
  
  [111] = 13548.1648246289,
  [112] = 17718.8064862859,
  [113] = 23173.3306586081,
  [114] = 30306.9653268588,
  [115] = 39636.6047183737,
  [116] = 51838.2628104401,
  [117] = 67796.0564558295,
  [118] = 88666.2673818679,
  [119] = 115961.124915207,
  [120] = 151658.380223528,
...
function StatLogic:GetEffectFromRating(rating, id, level, class)
	...
	if type(rating) ~= "number" or type(id) ~= "number" or (id < 1) or (id > CR_MAX) then return 0 end
	...
	return combatEffect, RatingIDToConvertedStat[id], ratingPerBonus; --e.g. 6.47162208, "PARRY"
end
...
function StatLogic:RemoveExtraSocketGem(link)
	...
	local _, _, _, _, _, _, _, _, itemType = GetItemInfo_dugi(link)
...
function StatLogic:GetGemID(item)
  ...
  -- Check if item is in local cache
  local name, link = GetItemInfo_dugi(item)
  if not name then
  ...
  if not GetItemInfo_dugi(6948) then -- Hearthstone
...
local function ItemIsTypeAndSubtype(item, isTypeIndex, isSubTypeIndex)
	local isLocalized = select(isSubTypeIndex,GetAuctionItemSubClasses(isTypeIndex))
	local itemLocalized = select(7, GetItemInfo_dugi(item))
...
function StatLogic:GetSum(item, table, threadingMode)
	...
	if bonusArmorItemEquipLoc[itemType] and table["ARMOR"] then
		-- Convert "ARMOR" to "BONUS_ARMOR"
		table["BONUS_ARMOR"] = (table["BONUS_ARMOR"] or 0) + table["ARMOR"]
		table["ARMOR"] = nil
	end
	
	--Racial Weapon Expertise
	local playerGainsRacial = 
		--maces
		((IsSpellKnown(59224) or IsSpellKnown(20864)) and (ItemIsTypeAndSubtype(item,1,5) or ItemIsTypeAndSubtype(item,1,6))) or
		--daggers
		(IsSpellKnown(92680) and ItemIsTypeAndSubtype(item,1,13)) or
		--swords
		(IsSpellKnown(20597) and (ItemIsTypeAndSubtype(item,1,8) or ItemIsTypeAndSubtype(item,1,9))) or
		--axes
		(IsSpellKnown(20574) and (ItemIsTypeAndSubtype(item,1,1) or ItemIsTypeAndSubtype(item,1,2) or ItemIsTypeAndSubtype(item,1,11)))
	if playerGainsRacial then
		table["EXPERTISE"] = 1
	end
...
function StatLogic:ParseLine(table, text, r,g,b)
	...
	--local excluded = true; --we'll assume everything was excluded, in case there are multiple stats on one line.
		...
		if not (found or L.Exclude[text] 
        or StatLogic.ItemsNotRecognized[text] 
        or L.Exclude[strutf8sub(text, 1, L.ExcludeLen)] 
        or strsub(text, 1, 1) == '"' or g < 0.8 or (b < 0.99 and b > 0.1)) then
			--debugPrint(text.." = ")
			-- Strip enchant time
					...
					if found then
						...
						if id ~= false then
							...
							if id == "ARMOR" and r == 0 and b == 0 and
								...
								table["BONUS_ARMOR"] = (table["BONUS_ARMOR"] or 0) + CNumber(value) - baseArmorTable[rarity][itemType][armorType][ilvl]
								debugText = debugText..", ".."|cffffff59ARMOR="..baseArmorTable[rarity][itemType][armorType][ilvl]..", BONUS_ARMOR="..(CNumber(value) - baseArmorTable[rarity][itemType][armorType][ilvl])
							else
								-- sum stat
								table[id] = (table[id] or 0) + CNumber(value)
								debugText = debugText..", ".."|cffffff59"..tostring(id).."="..tostring(value)
				...
				for _, sep in ipairs(L.DeepScanSeparators) do
					if strfind(text, sep) then
						...
						if strsub(sep, 1, 1) == "(" then
							...
							local beg,fin = strfind(text, sep) --gsub doesn't work properly w/ utf8 and regex
							while beg do
								local headAndMatch = strsub(text, 1, fin)
								local head = strutf8sub(headAndMatch, 1, beg-fin-2)
								local tail = strsub(text, fin+1)
								text = head.."@"..tail
								beg,fin = strfind(text, sep)
							end
							--text = gsub(text, sep, "@")
						end
					end
...
function StatLogic:GetFinalArmor(item, text)
	...
	-- Check if item is in local cache
	local name, _, rarity , ilvl, _, _, armorType, _, itemType = GetItemInfo_dugi(item)
	if not name then return end

	for pattern, id in pairs(L.PreScanPatterns) do
		if id == "ARMOR" or id == "BONUS_ARMOR" then
...
function StatLogic:GetDiffID(item, ignoreEnchant, ignoreGem, red, yellow, blue, meta, ignorePris)
	...
	-- Check if item is in local cache
	name, link, _, _, _, _, _, _, itemType = GetItemInfo_dugi(item)
	if not name then return end
	...
	if itemType == "INVTYPE_WEAPON" then
		...
		if IsUsableSpell(GetSpellInfo(674)) then        -- ["Dual Wield"]
			local _, _, _, _, _, _, _, _, eqItemType = GetItemInfo_dugi(linkDiff1)
