...
function NPCJournalFrame:Initialize()
	...
	function NPCJournalFrame:ReplaceWaypointTags(text, smallframe)
			...
			local buttonImage = "Interface\\AddOns\\DugisGuideViewerZ\\Artwork\\waypoint_16.tga"

			if self.currentHoveredWaypointButtonId == buttonId then
				buttonImage = "Interface\\AddOns\\DugisGuideViewerZ\\Artwork\\waypoint_y_16.tga"
			end
	...
	function NPCJournalFrame:CreateGUI()
		...
		self.strategyContent = CreateFrame("SimpleHTML",nil, contents[STRATEGY_TAB])
		...
		self.guideContent = CreateFrame("SimpleHTML",nil, contents[GUIDE_TAB])
		...
		LuaUtils:foreach({{name = "dPSContent", tab = DPS_TAB},
			...
			self[item.name] = CreateFrame("SimpleHTML",nil, contents[item.tab])
		...
		NPCJournalFrame.OnHyperlinkEnter = function(self, linkData, link, button, smallFrameAsOwner, stickyFrameAsOwner)
			...
			if tagType == "waypoint" then
				if not NPCJournalFrame.currentHoveredWaypointButtonId then
					NPCJournalFrame.needToUpdateWaypointButtons = true  --commented out as temporary fix, for some reason the waypoint no longer works if you change the image on OnHyperLinkHover. 
				end
		...
		NPCJournalFrame.OnHyperlinkLeave = function(self, linkData, link, button)
            ...
            NPCJournalFrame.needToUpdateWaypointButtons = true		  
		end
		...
		function Frame_OnUpdate(frame, elapsed)
			...
			if NPCJournalFrame.needToUpdateWaypointButtons then
				if NPCJournalFrame.guideTargetMode == TARGET_MODE then
					NPCJournalFrame:UpdateTarget(false, false, true) 
				else
					NPCJournalFrame:RefreshGuideData() 
				end
				NPCJournalFrame.needToUpdateWaypointButtons = nil
			end
	...
	function NPCJournalFrame:UpdateGuideTab(data, isMount, npcData, updateModel, isFollower, followerId)
		...
		if isMount then
            ...
            if self.guideContent.text ~= newText then
				...
				self.guideContent:SetText(self.guideContent.text)   
            ...
            if isFollower then
                ...
                if self.guideContent.text ~= newText then
                    self.guideContent.text = newText
                    self.guideContent:SetText(newText) 
                    UpdateLastTextChangePosition()
                end
            else
                local newText
	...
	function NPCJournalFrame:UpdateStrategyTab(npcData, updateModel)
        ...
        if self.strategyContent.text ~= newText then
            ...
            self.strategyContent:SetText(newText) 
            UpdateLastTextChangePosition()
        end        
	...
	function NPCJournalFrame:ReplaceQuestTagsInTexts()
		LuaUtils:foreach({self.strategyContent, self.dPSContent, self.healContent, self.tankContent, self.guideContent}, function(item)
			item:SetText(NPCJournalFrame:ReplaceSpecialTags(item.text))
		end)
	end
