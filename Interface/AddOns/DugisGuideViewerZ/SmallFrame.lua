...
function SmallFrame:Initialize()
	...
	function SmallFrame:PlayFlashAnimation(headerAnim)
		...
		if IsFloating() and DGV:UserSetting(DGV_SMALLFRAMEBORDER) then
			...
			if DGV:UserSetting(DGV_SMALLFRAMETRANSITION) == L["Flash"] and not DugisGuideViewer.Modules.DugisWatchFrame.disabledFlash then 
				LuaUtils:Delay(0.02, function() 
					DugisGuideViewer.Modules.DugisWatchFrame:PlayFlashAnimation(headerAnim) 
				end)
			end
    ...
    function UpdateProgressBarPosition()
        ...
        if (DugisGuideViewer:UserSetting(DGV_DISPLAYGUIDESPROGRESS)) and DugisGuideViewer:isValidGuide(CurrentTitle) == true then
            SmallFrameProgressBar:Show()
        else
            SmallFrameProgressBar:Hide()
        end  
        
        if (DugisGuideViewer:UserSetting(DGV_DISPLAYGUIDESPROGRESSTEXT)) and DugisGuideViewer:isValidGuide(CurrentTitle) == true then
	...
	SmallFrame.Load = function()
		...
		function DGV:OnWatchFrameUpdate()
			...
			then
				SmallFrame.Frame:Show()		
				if DugisGuideUser.ModelDataOn == true and DGV.Modules.ModelViewer.Frame and DugisSmallFrameStatus1ModelButton and DugisSmallFrameStatus1ModelButton:IsShown() then
					DGV.Modules.ModelViewer.Frame:Show()
				end
			else
				SmallFrameTooltip:Hide()
				if not IsFloating() then
					SmallFrame.Frame:Hide()
					if DugisGuideUser.ModelDataOn == true and DGV.Modules.ModelViewer.Frame then
						DGV.Modules.ModelViewer.Frame:Hide()
					end
				end
			end
