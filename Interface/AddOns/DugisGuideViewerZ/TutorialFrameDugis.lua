...
--/script TutorialFrameDugis_NewTutorial(1, true)
...
function GetFirstPossibleNotSeenTutorial()
...
function TryToShowTutorial1()
    ...
    if TutorialFrameDugis:IsShown() then
        ...
        if DISPLAY_DATA[TutorialFrameDugis.id].canBeDisplayed == nil or DISPLAY_DATA[TutorialFrameDugis.id].canBeDisplayed() then
            --Updating tutorial
            TutorialFrameDugis_NewTutorial(TutorialFrameDugis.id, true)
            return
        end
    end
    ...
    if firstPossibleIndex ~= nil then
        TutorialFrameDugis_NewTutorial(firstPossibleIndex, true)
    else
        TutorialFrameDugis:Hide()
    end
end
...
local ARROW_TYPES = {
 ...
 --/script TutorialFrameDugis_Update(14)
...
local function getFirstVisibleAbandonButton()
    ...
    LuaUtils:foreach(QuestMapFrame.QuestsFrame.Contents.Headers, function(parentButton)
            if firstVisibleButton == nil and parentButton.abandonGroupButton and parentButton.abandonGroupButton.button:IsVisible() then
...
You can also ]]..LightBlue("Right-Click ")..GetRightMouse()..[[ on the Small Frame to open the large menu]],
    ...
    [5] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisArrowFrame") end,
...
A ]]..GetGreenArrow()..[[ green waypoint arrow means you are in the questing area
    ...
    [6] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisGuideViewer_ModelViewer") end,
...
The Target Button ]]..GetTargetButton()..[[ will always match the NPC displayed in model viewer]],
    ...
    [7] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisGuideViewer_TargetFrame") end,
...
You can also keybind the Target Button by going to the WoW Key Bindings menu in the "Other" category]],
    ...
    [8] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisMainHomeTab") end,
   ...
   [9] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisMainHomeTab") and not DGVCurrentGuideFrame:IsVisible() end,
    ...
    [10] = { 
        ...
        anchorData = {align = "RIGHT", xOff = -250, yOff = 0},
        alignToParent = {parent = "DugisMainCurrentGuideTab"},
        canBeDisplayed = function() return IsFrameVisible("DGVCurrentGuideFrame") end,
        callOut = {parent = "DugisMainCurrentGuideTab", align = "TOPLEFT", xOff = -18, yOff = 5, width = 156, height = 50},
        --textBox = {topLeft_xOff = 33, topLeft_yOff = -145, bottomRight_xOff = -29, bottomRight_yOff = 35},
        --imageData1 = {file ="Interface\\TutorialFrame\\UI-TutorialFrame-AttackCursor", align = "TOP", xOff = -60, yOff = -60},
        --mouseData = {scale = 1.2, image = "LeftClick", align = "TOP", xOff = 50, yOff = -45},
    ...
    [13] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisResetButton")  end,
...
You will need to use this button for some guides like the Daily quests guides]],
    ...
    [14] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisReloadButton") end,
...
You should use this after clicking the ]]..WhiteText("Reset")..[[ button]],
    ...
    [16] = { 
        ...
        canBeDisplayed = function() return IsFrameVisible("DugisMainSettingsTab") and not (DugisGuideViewer.currentTabText == "Settings") end,
...
function AddText(parent, text, x, y, width, height, fontSize)
...
function TutorialFrameDugis_OnLoad(self)
    ...
    TutorialFrameCheckboxText:SetFontObject("GameFontHighlightSmall")
    TutorialFrameCheckboxText:SetPoint("TOPLEFT", 23,-7)
    checkbox.tooltip = "";
 
    TutorialFrameDugis_ClearTextures();
end
 
function TutorialFrameDugis_OnEvent(self, event, ...)
    if ( event == "TUTORIAL_TRIGGER" ) then
    elseif ( event == "DISPLAY_SIZE_CHANGED" ) then
        TutorialFrameDugis_Update(TutorialFrameDugis.id);
    elseif ( event == "PLAYER_ENTERING_WORLD" ) then
    elseif ( event == "LEARNED_SPELL_IN_TAB" ) then
    end
end
 
function TutorialFrameDugis_OnShow(self)
    self:RegisterEvent("DISPLAY_SIZE_CHANGED");
    TutorialFrameDugis_CheckNextPrevButtons();
end
 
function TutorialFrameDugis_OnHide(self)
    self:UnregisterEvent("DISPLAY_SIZE_CHANGED");
end
 
function TutorialFrameDugis_OnKeyDown(self, key)
    ...
    if (displayData["keyData1"]) then
        ...
        if (allOff) then
            TutorialFrameDugis_Hide();
        end
    end
end
 
function TutorialFrameDugis_OnMouseDown(self, button)
...
function TutorialFrameDugis_CheckNextPrevButtons()
...
function TutorialFrameDugis_Update(currentTutorial)
...
function TutorialFrameDugis_ClearTextures()
    TutorialFrameDugis:ClearAllPoints();
...
function TutorialFrameDugis_NewTutorial(tutorialID, forceShow)
    ...
    if(forceShow) then
        TutorialFrameDugis_Update(tutorialID);
        return;
    end
...
function TutorialFrameDugisPrevButton_OnClick(self)
    ...
    if prevTutorial then
        TutorialFrameDugis_Update(prevTutorial);
    end
end
 
function TutorialFrameDugisNextButton_OnClick(self)
    if TutorialFrameCheckbox:GetChecked() then
        TutorialFrameDugisHidden = true
        TutorialFrameCheckbox:SetChecked(false)
        TutorialFrameDugis_Hide()
    ...
    if nextTutorial then
        TutorialFrameDugis_Update(nextTutorial);
    end
end
 
function TutorialFrameDugis_AlertButton_OnClick(self)
end
 
function TutorialFrameDugis_Hide(stopTutorial)
...
function TutorialFrameDugis_CheckBadge()
