...
hopAddon.buttonChangeMode:SetScript("OnClick", function(btn)
	...
	if hopAddon.hostFrame:IsShown() then
		hopAddon.hostFrame:Hide()
		hopAddon.searchFrame:Show()
	else
		hopAddon.hostFrame:Show()
...
local function ServerHop_EventSystem(self, event, ...)
	...
	if event == "ADDON_LOADED" and arg1 == "ServerHop" then	
		...
		if ServerHopSettingsGlobal ~= nil then
			if ServerHopSettingsGlobal["ALERT_ON_ZONE"] ~= nil then
				hopAddon.optionsFrame.hostOptionsFrame.showAlertsZone:SetChecked(ServerHopSettingsGlobal["ALERT_ON_ZONE"])
			end

			if ServerHopSettingsGlobal["ALERT_ON_TIME"] ~= nil then
				hopAddon.optionsFrame.hostOptionsFrame.showAlertsTime:SetChecked(ServerHopSettingsGlobal["ALERT_ON_TIME"])
			end
		end
	...
	elseif event == "PLAYER_LOGOUT" then
		...
		ServerHopSettingsGlobal["ALERT_ON_ZONE"] = hopAddon.optionsFrame.hostOptionsFrame.showAlertsZone:GetChecked()
		ServerHopSettingsGlobal["ALERT_ON_TIME"] = hopAddon.optionsFrame.hostOptionsFrame.showAlertsTime:GetChecked()
