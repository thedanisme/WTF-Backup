...
hopAddon.searchFrame.refreshButton = CreateFrame("Button", nil, hopAddon.searchFrame,"BrowserButtonTemplate")
...
hopAddon.searchFrame.refreshButton:Hide()

-- search results scroll
hopAddon.searchFrame.scrollframe = CreateFrame("ScrollFrame", "SH_SearchResultsScroll", hopAddon.searchFrame, "UIPanelScrollFrameTemplate");
hopAddon.searchFrame.scrollframe:SetPoint("TOP",hopAddon.searchFrame,"BOTTOM",0,-20)
hopAddon.searchFrame.scrollframe:SetSize(HOPADDON_WIDTH,225)

hopAddon.searchFrame.scrollframe.scrollchild = CreateFrame("Frame")
local scrollbarName = hopAddon.searchFrame.scrollframe:GetName()
hopAddon.searchFrame.scrollframe.scrollchild:SetSize(hopAddon.searchFrame.scrollframe:GetWidth(), hopAddon.searchFrame.scrollframe:GetHeight());

hopAddon.searchFrame.scrollframe:SetScrollChild(hopAddon.searchFrame.scrollframe.scrollchild);

hopAddon.searchFrame.scrollframe.scrollupbutton = _G[scrollbarName.."ScrollBarScrollUpButton"];
hopAddon.searchFrame.scrollframe.scrollupbutton:ClearAllPoints();
hopAddon.searchFrame.scrollframe.scrollupbutton:SetPoint("TOPRIGHT", hopAddon.searchFrame.scrollframe, "TOPRIGHT", 0, -5);

hopAddon.searchFrame.scrollframe.scrolldownbutton = _G[scrollbarName.."ScrollBarScrollDownButton"];
hopAddon.searchFrame.scrollframe.scrolldownbutton:ClearAllPoints();
hopAddon.searchFrame.scrollframe.scrolldownbutton:SetPoint("BOTTOMRIGHT", hopAddon.searchFrame.scrollframe, "BOTTOMRIGHT", 0, 0);

hopAddon.searchFrame.scrollframe.scrollbar = _G[scrollbarName.."ScrollBar"];
hopAddon.searchFrame.scrollframe.scrollbar:ClearAllPoints();
hopAddon.searchFrame.scrollframe.scrollbar:SetPoint("TOP", hopAddon.searchFrame.scrollframe.scrollupbutton, "BOTTOM", 0, 4);
hopAddon.searchFrame.scrollframe.scrollbar:SetPoint("BOTTOM", hopAddon.searchFrame.scrollframe.scrolldownbutton, "TOP", 0, -4);
...
local function UpdateSearchFrames()
	...
	if #FilteredGroupsList <= 7 then
		hopAddon.searchFrame.scrollframe.scrollupbutton:Hide()
		hopAddon.searchFrame.scrollframe.scrolldownbutton:Hide()
		hopAddon.searchFrame.scrollframe.scrollbar:Hide()
	end
...
local function InitiateRefresh()
	hopAddon.searchFrame.refreshButton:Hide()
	hopAddon.searchFrame.scrollframe:Hide()
	_G[hopAddon.searchFrame.searchBox:GetName().."SearchIcon"]:Show()
...
local function RemoveDelisted()
	...
	while i > 0 do		
		if FoundInDelisted(FilteredGroupsList[i]) then
			ResultFrames[i]:Hide()
			--print("Delisted "..select(3,C_LFGList.GetSearchResultInfo(FilteredGroupsList[i])))
...
local function FilterCustomSearchResults()
	...
	if #FilteredGroupsList > 0 then
		...
		searchUpdated = 0
		UpdateSearchFrames()
		hopAddon.searchFrame.refreshButton:Show()
		...
		if (hopAddon.optionsFrame.globalOptionsFrame.chatNotifButton:GetChecked()) then
			local id, action, caption, desc, voice, ilvl, time, bnetfr, charfr, guild, delisted, fullname, pcount, autoinv = C_LFGList.GetSearchResultInfo(FilteredGroupsList[1])
			local realm = PlayerRealm(fullname)
			if autoinv then ncolor = "3caa3c" end
			print("[Server Hop] ["..SEARCH.."]: \124cff71d5ff\124HsehopR:*"..C_LFGList.GetActivityInfo(action).."\n"..HOPADDON_AUTOINVITE.."*"..desc.."*\124h["..caption.."]\124h\124r.");
		end
		]]
...
local dDropList = {
	...
	-- Legion
	[425] = 100,[435] = 200,[445] = 300,
	[426] = 100,[436] = 200,[446] = 300,
	[427] = 100,[437] = 200,[447] = 300,
	[428] = 100,[438] = 200,[448] = 300,
	[429] = 100,[439] = 200,[449] = 300,
	[430] = 100,[440] = 200,[450] = 300,
	[431] = 100,[441] = 200,[451] = 300,
	[432] = 100,[442] = 200,[452] = 300,
	[433] = 100,[443] = 200,[453] = 300,
...
hopAddon.searchFrame.refreshButton:SetScript("OnClick", function(btn)
...
local function MakeResultFrame(index)
	local frame = CreateFrame("Button",nil,hopAddon.searchFrame.scrollframe.scrollchild)
	frame:SetSize(228,28)
	if index == 1 then
	...
	frame:SetScript("OnClick", function(self,button,down)
		...
		if button == "LeftButton" then
			...
			if total < 5 and ((not IsInGroup() and not IsInRaid()) or UnitIsGroupLeader('player')) and not name then
				...
				if #FilteredGroupsList > 0 then
					UpdateSearchFrames()
					-- switching tooltip to the frame that came to the cursor up from below in the list
